...
class Under100DollarsItemsPagingSource constructor(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Product> {
        ...
        return when (val response = service.fetchUnder100DollarsItems(queryMap, request)) {
            is ApiResponse.ApiSuccessResponse -> {
                val under100DollarsResponse = response.data!!.getResponse<ProductPagingListing>()
                onProductListingResponse?.invoke(under100DollarsResponse)
                val (prevKey, nextKey) = getPagingKeys(under100DollarsResponse.pagingInfo)
                LoadResult.Page(under100DollarsResponse.products, prevKey, nextKey)
            }
